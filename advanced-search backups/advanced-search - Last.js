...
// Il faut trouver comment extraire le range dans findSearchTermPassage
// Il faut encore modifier la partie pour le pali dans searchSuttasWithStop
// Problème d'highlight avec strict (la dernière lettre n'est pas prise en compte)
...
function findSearchTermPassages(textData, searchTerm, multipleVerse = true, strict = false, pali = false) {
	...
	const searchTermWords = lowerCaseSearchTerm.split(" ");
	...
	const findInSingleVerse = (verse, verseIndex, allVerses) => {
		const matches = [];
		let match;
		...
		while ((match = searchTermRegex.exec(verse.toLowerCase())) !== null) {
			...
			const searchTermLengthInWords = searchTermWords.length;
			...
			// Check for multi-verse term matching
			const nextVerse = allVerses[verseIndex + 1];
			if (nextVerse) {
				const nextVerseWords = nextVerse.split(" ");
				const combinedSearchTerm = searchTermWords.slice(1).join(" "); // Ignore the first word for the next verse match
				const nextVerseMatch = new RegExp(`^${combinedSearchTerm}`, "i");
				
				if (nextVerseMatch.test(nextVerse)) {
					// If there is a match in the next verse, extract from both
					const nextMatchIndex = nextVerse.search(nextVerseMatch);
					const combinedStartIdx = Math.max(0, termWordIndex - Math.floor((maxWords - searchTermLengthInWords) / 2));
					const combinedEndIdx = Math.min(allWords.length, combinedStartIdx + maxWords + nextVerseWords.length);

					matches.push(extractPassage(
						words.concat(nextVerseWords),
						combinedStartIdx,
						combinedEndIdx,
						matchIndex,
						verse + " " + nextVerse
	...
	if (!multipleVerse) {
		for (const [, verse] of verses) {
			const passages = findInSingleVerse(verse, line - 1, verses);
			if (passages) {
		...
		while ((match = searchTermRegex.exec(concatenatedText.toLowerCase())) !== null) {
			...
			let startIdx = Math.max(0, termWordIndex - Math.floor((maxWords - searchTermWords.length) / 2));
